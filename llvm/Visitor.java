...
public class Visitor extends IRBuilder {
    ...
    private Visitor() {
        ...
        curSymTable.addSymbol("getint", Module.getInt);
        curSymTable.addSymbol("putint", Module.putInt);
        curSymTable.addSymbol("putch", Module.putChar);
        curSymTable.addSymbol("putstr", Module.putStr);
    }
    ...
    public void visitConstDef(ConstDef constDef) {
        ...
        boolean isArray = (constExp != null) ? true : false;
        if (isArray) {
    ...
    public void visitPrintfSubStmt(PrintfSubStmt printfSubStmt) {
        ...
        for (String string : strings) {
            if (string.equals("%d")) {
                ...
                temp.add(vRparm);
                makeCall(Module.putInt, temp);   // 调用putint函数输出
            } else {
