...
//        GlobalVar globalvar = new GlobalVar(varName, type, init, isConst);
    ...
    // TODO这个init到底有什么用   ------ 用于constExp在编译期的计算
    public Alloca makeAlloca(LLVMType type, Constant init) {   // const变量的区域存入init
        ...
        // 将alloca指令移动到curFunc的entry块中
        curFunc.getFirstBasicBlock().addInstr2Head(alloca);
        alloca.setHost(curFunc.getFirstBasicBlock());
        return alloca;
    }
...
//        basicBlock.addInstruction(new Compute(namecnt++, new Token(TokenType.PLUS, " ", -1), new ConstantData(LLVMType.INT32, 0), new ConstantData(LLVMType.INT32, 0)));
    ...
    public Getelemntptr makeGetelemntptr(Value base_value) {
        ...
        getelemntptr.setHost(curBBlock);
        return getelemntptr;
    }
    ...
    public Zext makeZext(Value oldValue, LLVMType targetType) {
        ...
        zext.setHost(curBBlock);
        return zext;
    }


    /// -----Optimize-----
    private static int phiCnt = 0;

    public static Phi makePhi(BasicBlock host, LLVMType type) {
        Phi phi = new Phi(phiCnt++, type, new ArrayList<>(host.getFrontBBlocks()));
        host.addInstr2Head(phi);
        phi.setHost(host);
        return phi;
    }
