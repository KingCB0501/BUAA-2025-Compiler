...
public class MipsGenerator extends MipsBuilder {
    ...
    public void tranGetelemntptr(Getelemntptr getelemntptr) {
        ...
        if (offestValue instanceof ConstantData voff) {
            ...
            offestReg = findReg4Value(offestValue);
            makeSll(offestReg, offestReg, 2);    // TODO  此处会直接更改原寄存器的值，需要保存原来寄存器的值吗
        } else {
...
//            System.out.println(offest);
    ...
    public void tranCall(Call call) {
        ...
        ArrayList<Reg> allocatedRegs = getAllocatedRegs();
        int offset = 0;
        for (int i = 0; i < allocatedRegs.size(); i++) {
            offset = offset - 4;     // 保存用到的寄存器
            makeSw(allocatedRegs.get(i), offset + curOffest, Reg.sp);
        }
        offset = offset - 4;
        ...
        for (int i = 0; i < rParams.size(); i++) {
            ...
            if (i < 4) {   // TODO 目前实现中所有数据在栈上都能找到
                Reg argReg = Reg.getArgReg(i);
                if (arg instanceof ConstantData argNum) {
                    makeLi(argReg, argNum.getNum());
                } else if (findReg4Value(arg) != null && !findReg4Value(arg).isParamReg()) {    // TODO如何保证栈与寄存器数据同步
                    makeAdd(argReg, findReg4Value(arg), Reg.zero);
                } else {
        ...
        for (int i = 0; i < allocatedRegs.size(); i++) {
            offset = offset - 4;
            makeLw(allocatedRegs.get(i), offset + curOffest, Reg.sp);
        }
